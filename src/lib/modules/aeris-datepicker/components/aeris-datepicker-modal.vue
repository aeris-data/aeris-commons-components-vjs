<i18n src="../assets/dates.json"></i18n>

<template>
  <span v-click-outside="validate" :style="applyTheme" class="aeris-datepicker-host">
    <div class="dp-container unselectable">
      ...
      <footer class="dp-footer">
        <button v-if="hasToday" class="today-button" @click.prevent="setToToday">{{ $t("today") }}</button>
        <button id="validate-btn" @click.prevent="validate">{{ $t("validate") }}</button>
      </footer>
    </div>
  </span>
</template>
...
export default {
  ...
  created() {
    ...
    this.setDate(moment(this.dateFromInput, this.format));
  },
  ...
  methods: {
    ...
    sendDateToParent() {
      let value = this.currentDate.format(this.format);
    ...
    clickDay(e) {
      ...
      if (isClickable) {
        ...
        let newDate = date.format(this.format);
